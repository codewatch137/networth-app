...
import { DatePicker, message, Spin, Tag } from "antd";
...
interface Client {
  ...
  dob?: string; permanent_address?: string; mobile?: string;
}
...
const sumRows = (rows: any[], field: string) => rows.reduce((s: number, r: any) => s + (Number(r[field]) || 0), 0);

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function toRows<T extends Record<string, any>>(arr: T[]): (T & { key: string })[] {
  return (arr || []).map((item, i) => ({ ...item, key: item.key || String(i) }));
}
...
function Donut({ groups }: { groups: { label: string; value: number; color: string }[] }) {
  const size = 100, r = 36, cx = 50, cy = 50, circ = 2 * Math.PI * r;
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
      <svg width={size} height={size} style={{ flexShrink: 0 }}>
        ...
        {groups.map((g, i) => {
          ...
          const el = <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={g.color} strokeWidth="14"
  ...
  return (
    <div className="card summary-card">
      ...
      <div className="card-body">
        ...
        <div className="summary-row"><span>(A) Immovable</span><b className="num">{formatINRCompact(totalA)}</b></div>
        <div className="summary-row indent"><span>By Self</span><span className="num">{formatINRCompact(totalSelf)}</span></div>
        <div className="summary-row indent"><span>By Sharing</span><span className="num">{formatINRCompact(totalSharing)}</span></div>
        <div className="summary-row"><span>(B) Other Assets</span><b className="num">{formatINRCompact(totalOther)}</b></div>
        <div className="summary-row"><span style={{ fontWeight: 500 }}>Total Assets (A+B)</span><b className="num">{formatINRCompact(totalAssets)}</b></div>
        <div className="summary-row"><span>(C) Liabilities</span><b className="num" style={{ color: "var(--danger)" }}>−{formatINRCompact(totalC)}</b></div>
        {totalGuaranteed > 0 && (
          <div className="summary-row"><span className="muted">Guarantees (memo)</span><span className="num muted">{formatINRCompact(totalGuaranteed)}</span></div>
        )}
      </div>
    </div>
  );
    ...
    <div className="steps">
      {STEPS.map((label, i) => (
        <div key={label} style={{ display: "flex", alignItems: "center", flex: i < STEPS.length - 1 ? 1 : 0 }}>
          <button className={`step ${i === current ? "active" : i < current ? "done" : ""}`}
            onClick={() => onJump(i)} style={{ background: "none", border: "none" }}>
...
function FormFooter({ onBack, onNext, nextLabel = "Continue →", saving = false }: {
  ...
  return (
    <div className="form-footer">
      ...
      {onNext && <button className="btn btn-brand" onClick={onNext} disabled={saving}>{saving ? "Saving…" : nextLabel}</button>}
    </div>
  );
}

/* ── Collapsible immovable row ──────────────────────────────── */
function ImmovableRowEditor({ row, idx, onUpdate, onRemove }: {
  row: ImmovableRow; idx: number;
  onUpdate: (r: ImmovableRow) => void; onRemove: () => void;
}) {
  ...
  return (
    <div className="detail-row">
      <div className="detail-row-head" onClick={() => setOpen((o) => !o)}>
        ...
        <button className="btn btn-icon btn-ghost" onClick={(e) => { e.stopPropagation(); onRemove(); }}>
        ...
        <svg width="14" height="14" viewBox="0 0 16 16" fill="none" style={{ color: "var(--text-3)", flexShrink: 0 }}>
          {open ? <path d="M4 10l4-4 4 4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
            ...
            <div className="detail-grid">
              ...
              <div className="field">
                ...
                <input className="input" type="number" step="0.01" style={{ textAlign: "right" }}
                  value={row.propertyCost || ""} onChange={(e) => set("propertyCost", Number(e.target.value))}/>
              </div>
              <div className="field">
                <label className="label">Registration Charges (₹)</label>
                <input className="input" type="number" step="0.01" style={{ textAlign: "right" }}
                  value={row.registrationCharges || ""} onChange={(e) => set("registrationCharges", Number(e.target.value))}/>
              </div>
              <div className="field">
                <label className="label">Stamp Charges (₹)</label>
                <input className="input" type="number" step="0.01" style={{ textAlign: "right" }}
                  value={row.stampCharges || ""} onChange={(e) => set("stampCharges", Number(e.target.value))}/>
              </div>
              <div className="field field-full">
...
/* ── Bucket block ───────────────────────────────────────────── */
function BucketBlock({ title, subtitle, items, onUpdate, onRemove, onAdd }: {
...
/* ── Movable row (collapsible w/ source of fund) ────────────── */
function MovableRowEditor({ row, idx, onUpdate, onRemove }: {
  row: MovableRow; idx: number;
  onUpdate: (r: MovableRow) => void; onRemove: () => void;
}) {
  const [open, setOpen] = useState(false);
...
/* ── Category block (collapsible w/ holders) ────────────────── */
...
function LiabRow({ row, onUpdate, onRemove }: {
  row: LiabilityRow; onUpdate: (r: LiabilityRow) => void; onRemove: () => void;
}) {
...
/* ── Guarantor row ──────────────────────────────────────────── */
function GuarantorRow({ row, onUpdate, onRemove }: {
  row: GuarantorRow; onUpdate: (r: GuarantorRow) => void; onRemove: () => void;
}) {
      ...
      <div style={{ border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
        {items.map(([k, v], i) => (
          <div key={k} style={{
            ...
            fontWeight: highlight && i === items.length - 1 ? 600 : 400, fontSize: 13,
          }}>
...
export default function EditCertificatePage() {
  ...
  const [certStatus, setCertStatus] = useState<"draft" | "issued">("draft");
  const [showAmendNotice, setShowAmendNotice] = useState(true);
  ...
  useEffect(() => {
    fetch(`/api/certificates/${certId}`)
      ...
      .then((d) => {
        ...
        if (!cert) { message.error("Certificate not found"); return; }

        setClient(cert.clientId);
        setCertNumber(cert.certNumber || "");
        setCertStatus(cert.status === "issued" ? "issued" : "draft");
        setIssueDate(cert.issueDate ? dayjs(cert.issueDate) : dayjs());
        setAsOnDate(cert.asOnDate ? dayjs(cert.asOnDate) : null);
        if (cert.purpose) setPurpose(cert.purpose);

        const allImm: ImmovableRow[] = toRows(cert.immovableProperties || []).map((r) => ({
          key: r.key, ownershipType: r.ownershipType || "self",
          nature: r.nature || "", address: r.address || "",
          dateOfPurchase: r.dateOfPurchase || "",
          propertyCost: r.propertyCost || r.valueAtCost || 0,
          registrationCharges: r.registrationCharges || 0,
          stampCharges: r.stampCharges || 0,
          vendor: r.vendor || "",
          loanBank: r.loanBank || "",
          loanReceived: r.loanReceived || 0,
          sanctionRef: r.sanctionRef || "",
          loanDate: r.loanDate || "",
          loanOutstanding: r.loanOutstanding || r.outstandingLoan || 0,
          srcSalary: r.srcSalary || 0,
          srcSB: r.srcSB || 0,
          srcFD: r.srcFD || 0,
          srcOther: r.srcOther || 0,
        }));
        setImmSelf(allImm.filter((r) => r.ownershipType === "self"));
        setImmSharing(allImm.filter((r) => r.ownershipType === "sharing"));

        const mapMovable = (arr: Record<string, unknown>[]): MovableRow[] =>
          toRows(arr || []).map((r) => ({
            key: r.key, holder: (r.holder as string) || "Self",
            refNo: (r.refNo as string) || "",
            heldWith: (r.heldWith as string) || (r.description as string) || "",
            dateOfInvestment: (r.dateOfInvestment as string) || "",
            value: Number(r.value ?? r.presentValue) || 0,
            soldAmount: Number(r.soldAmount) || 0,
            soldDate: (r.soldDate as string) || "",
            loanBank: (r.loanBank as string) || "",
            loanReceived: Number(r.loanReceived) || 0,
            sanctionRef: (r.sanctionRef as string) || "",
            loanDate: (r.loanDate as string) || "",
            loanOutstanding: Number(r.loanOutstanding) || 0,
            srcSalary: Number(r.srcSalary) || 0,
            srcSB: Number(r.srcSB) || 0,
            srcFD: Number(r.srcFD) || 0,
            srcOther: Number(r.srcOther) || 0,
          }));

        // Group flat array → Record<holder, MovableRow[]> based on category's holders config
        const groupByHolder = (rows: MovableRow[], catId: string): Record<string, MovableRow[]> => {
          const cat = OTHER_ASSET_CATS.find((c) => c.id === catId)!;
          const out: Record<string, MovableRow[]> = Object.fromEntries(cat.holders.map((h) => [h, []]));
          rows.forEach((r) => {
            const h = cat.holders.includes(r.holder) ? r.holder : cat.holders[0];
            out[h].push(r);
          });
          return out;
        };

        const next: OtherAssetData = buildEmptyOtherAssets();
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const c = cert as any;
        const mappings: Record<string, Record<string, unknown>[]> = {
          ppf:                   c.ppf || [],
          pension:               c.pensionScheme || [],
          huf:                   c.huf || [],
          shares:                c.shares || [],
          fd:                    c.fixedDeposit || [],
          rd:                    c.recurringDeposit || [],
          otherDeposit:          c.otherDeposit || [],
          insurance:             c.insurancePolicies || [],
          vehicleTwo:            c.vehicleTwoWheeler || (Array.isArray(c.vehicles) ? c.vehicles : []),
          vehicleFour:           c.vehicleFourWheeler || [],
          capitalProprietorship: c.capitalProprietorship || [],
          capitalFirm:           c.capitalFirm || [],
          cash:                  c.cashInHand || [],
          gold:                  c.gold || [],
          mf:                    c.mutualFunds || [],
        };
        Object.entries(mappings).forEach(([catId, arr]) => {
          next[catId] = groupByHolder(mapMovable(arr), catId);
        });
        setOtherAssets(next);

        setLiabilities(toRows(cert.liabilities || []).map((r) => ({
          key: r.key, borrowedFrom: (r.borrowedFrom as string) || "",
          securities: (r.securities as string) || "",
          purpose: (r.purpose as string) || "",
          outstandingAmount: Number(r.outstandingAmount) || 0,
        })));

        setGuarantors(toRows(cert.guarantors || []).map((r) => ({
          key: r.key,
          guaranteedTo: (r.guaranteedTo as string) || "",
          borrowingsBy: (r.borrowingsBy as string) || "",
          purpose: (r.purpose as string) || "",
          amountGuaranteed: Number(r.amountGuaranteed) || 0,
        })));
      })
  ...
  const buildPayload = (status: "draft" | "issued") => {
    ...
    return {
      asOnDate: asOnDate?.toISOString(), issueDate: issueDate?.toISOString(), purpose, status,
  ...
  /** PUT the current state. If `next` provided, advance step on success but stay on page.
   *  If `redirect` is true, navigate to the certificate detail page on success (used by Save draft / Issue). */
  const persist = async (status: "draft" | "issued", opts: { next?: number; redirect?: boolean; silent?: boolean } = {}) => {
    if (!asOnDate) { message.error("Please select the 'Net worth as on' date"); return false; }
    setSaving(true);
    try {
      ...
      if (opts.redirect) router.push(`/certificates/${certId}`);
      else if (opts.next !== undefined) setStep(opts.next);
      return true;
    } catch (err: unknown) {
  ...
  const handleSave = (status: "draft" | "issued") => persist(status, { redirect: true });
  // While stepping through an already-issued cert we must NOT silently demote it to draft.
  const saveAndNext = (next: number) => persist(certStatus, { next, silent: true });

  if (loading) return <div className="flex justify-center" style={{ paddingTop: 64 }}><Spin/></div>;
  if (!client)  return (
    <div style={{ padding: 40, textAlign: "center", color: "var(--text-3)" }}>
      <div style={{ fontWeight: 600, marginBottom: 8, color: "var(--text)" }}>Cannot edit this certificate</div>
      <div className="text-sm">The client linked to this certificate has been deleted. Restore or recreate the client record before editing.</div>
      <button className="btn btn-sm" style={{ marginTop: 16 }} onClick={() => router.back()}>← Go back</button>
    </div>
  );
  ...
  const steps: React.ReactNode[] = [
    ...
    <div key="purpose" className="card">
      ...
      <FormFooter onNext={() => saveAndNext(1)} saving={saving}/>
    </div>,
    ...
    <div key="immovable" className="card">
      ...
      <BucketBlock title="By Self" subtitle="Properties owned 100% by the client" items={immSelf}
      ...
      <BucketBlock title="By Sharing" subtitle="Properties co-owned with spouse, family, or partners" items={immSharing}
      ...
      <FormFooter onBack={() => setStep(0)} onNext={() => saveAndNext(2)} saving={saving}/>
    </div>,
    ...
    <div key="other" className="card">
      ...
      <FormFooter onBack={() => setStep(1)} onNext={() => saveAndNext(3)} saving={saving}/>
    </div>,
    ...
    <div key="liabilities" className="card">
      <div className="section-header">
        <div className="flex items-center gap-12">
          ...
          <div><h4>(C) Total Liabilities</h4><p>Borrowings outstanding as on the certificate date.</p></div>
        </div>
      </div>
      ...
      <FormFooter onBack={() => setStep(2)} onNext={() => saveAndNext(4)} saving={saving}/>
    </div>,
    ...
    <div key="guarantees" className="card">
      <div className="section-header">
        <div className="flex items-center gap-12">
          ...
          <div><h4>Guarantees given by client</h4><p>Borrowings by friends/relatives/firms for which the client is guarantor.</p></div>
        </div>
      ...
      <FormFooter onBack={() => setStep(3)} onNext={() => saveAndNext(5)} saving={saving}/>
    </div>,
    ...
    <div key="review" className="card">
      <div className="section-header">
        <div className="flex items-center gap-12">
          ...
          <div><h4>Review &amp; issue</h4><p>Confirm all details before updating the certificate.</p></div>
        </div>
      </div>
      <div className="card-body">
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div>
          <button className="btn btn-ghost btn-sm" onClick={() => router.push(`/certificates/${certId}`)} style={{ marginBottom: 8, paddingLeft: 0 }}>
            ← Back
          </button>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <h2 style={{ margin: 0 }}>Edit Certificate</h2>
            {certNumber && <span style={{ fontSize: 13, fontFamily: "var(--font-mono)", color: "var(--text-3)" }}>{certNumber}</span>}
            <Tag color={certStatus === "issued" ? "green" : "orange"} style={{ fontSize: 10 }}>
              {certStatus === "issued" ? "Issued" : "Draft"}
            </Tag>
          </div>
        ...
        <div className="page-header-actions">
          {certStatus === "issued" ? (
            ...
            <>
              <button className="btn" onClick={() => handleSave("draft")} disabled={saving}>Save draft</button>
              <button className="btn btn-brand" onClick={() => saveAndNext(5)} disabled={saving}>Review →</button>
            </>
          )}
        </div>
      </div>

      {certStatus === "issued" && showAmendNotice && (
        <div style={{
          margin: "12px 0 16px",
          padding: "10px 14px",
          background: "#fef3c7",
          ...
          <span style={{ flex: 1 }}>
            <b>You&apos;re editing an issued certificate.</b> Saving will overwrite the locked record — only do this to fix mistakes. The original issue date is preserved.
          </span>
          <button
            type="button"
            aria-label="Dismiss"
            onClick={() => setShowAmendNotice(false)}
            style={{
              flexShrink: 0,
            ...
            onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.opacity = "1"; (e.currentTarget as HTMLElement).style.background = "rgba(146,64,14,0.1)"; }}
      ...
      <div className="split-2-1">
        <div>{steps[step]}</div>
        <SummaryPanel
          totalSelf={totalSelf} totalSharing={totalSharing}
          totalOther={totalOther} totalC={totalC}
